...
train_ir, train_rr, sample_ir, sample_rr, test_ir, test_rr = get_nc_list(ROOT_DATA_PATH)
...
if LOAD is True:
    check_point = torch.load(os.path.join(MODEL_SAVE_PATH, MODEL_NAME + '.pt'))
    ...
    try:
        if MODE == 'train':
            for i in range(EPOCH):
                ...
                torch.save(model.state_dict(), os.path.join(MODEL_SAVE_PATH, MODEL_NAME + '.pt'))
                ...
                cuda.memory_reserved(DEVICE)
                cuda.max_memory_reserved(DEVICE)
